...
3. Fill in the form:
  ...
  | **Role** | `dev-eks-devops-group-EKSAdminFullAccessRole` |
   | **Display name** | `EKS Admin` *(optional label)* |
4. Click **Switch Role** — the console session now runs under `dev-eks-devops-group-EKSAdminFullAccessRole`
...
```bash
# Update kubeconfig to use the group admin role
aws eks update-kubeconfig \
  --name <cluster_name> \
  --region <aws_region> \
  --role-arn arn:aws:iam::<account_id>:role/<cluster_name>-devops-group-EKSAdminFullAccessRole \
...
If an IAM group already exists outside Terraform (for example `devops`), import it before first apply to avoid `EntityAlreadyExists`:

```bash
terragrunt import 'aws_iam_group.devops_eks_admin["devops"]' devops
```
